...
| 2026-07-19 | By explicit user direction, accept the Claude and Codex Task 25 `workstack-quick-task` and `workstack-start` clean-session probes as PASS; mark both remaining `workstack-resume` probes and all three Task 26 pilots WAIVED, not PASS or FAIL. No pilot-exposed specification gaps are claimed because no pilot ran. | The user waived the remaining behavioral evidence and prohibited replacement pressure scenarios or behavioral evals. Deterministic coverage remains evidence only for the tested contracts, not a substitute pilot verdict. |
| 2026-07-19 | Task 27 lands as one direct commit on FSMCRM `workstack/s6-cutover` and one user-authorized direct commit on fork `main`; no PR is opened, and the controller retains independent final review, integration, and cleanup ownership. Bounded repository checks found no Task 25/26 pilot plan, active contract, PR branch/worktree, or assigned Linear ticket to reconcile. | Avoids fabricating cleanup or external Linear work for artifacts that were never created while preserving the active FSMCRM worktree and fork plan through final S6 review. |
...
#### Spec §23 evidence checklist (Task 27 closeout)

`SATISFIED` means current deterministic or repository evidence establishes the condition. `WAIVED` records a missing behavioral gate excused by explicit user direction; it is not a PASS or FAIL.

1. [x] **SATISFIED — project-local installation.** FSMCRM `skills-lock.json` records 21 `mlk1278/superpowers-workstack` skills; the Task 27 mirror inventory confirms all 21 in both `.claude/skills` and `.codex/skills`, with no global/project plugin dependency. Fork evidence: `tests/codex/test-package-codex-plugin.sh`, `tests/codex/test-marketplace-manifest.sh`, and `tests/codex-plugin-sync/test-sync-to-codex-plugin.sh`.
2. [ ] **WAIVED — three clean-session entry points.** Accepted PASS evidence: FSMCRM ignored verdicts `.superpowers/evals/task-25-{claude,codex}-{quick-task,start}.md`. Both `workstack-resume` probes are WAIVED by explicit user direction; their prior `NOT RUN` files are not promoted to PASS.
3. [x] **SATISFIED — canonical locations.** `tests/workstack/test-plan-docs.sh`, `tests/workstack/test-active-contracts.sh`, `tests/workstack/test-agent-routing.sh`, and `tests/workstack/test-quick-task.sh` enforce plan, contract, routing, ledger, worktree-context, and scratch locations.
4. [x] **SATISFIED — living-plan shape.** `tests/workstack/test-plan-docs.sh` covers deferred checkpoints and slice states; `tests/workstack/test-resume.sh` covers checkpoint materialization and multi-slice dispatch states.
5. [ ] **WAIVED — complete sequential delivery behavior.** Deterministic contracts pass in `tests/workstack/test-final-review-gate.sh`, `test-reviewer-context.sh`, `test-ux-gate.sh`, `test-slice-gate.sh`, `test-pr-monitor.sh`, `test-quick-task.sh`, and `test-resume.sh`; the Task 26 sequential multi-ticket pilot is WAIVED, so no end-to-end behavioral PASS is claimed.
6. [ ] **WAIVED — parallel-slice behavior.** Deterministic isolation and contract rules pass in `tests/workstack/test-active-contracts.sh` and `tests/workstack/test-resume.sh`; the Task 26 parallel-slices pilot is WAIVED, so no behavioral PASS is claimed.
7. [x] **SATISFIED — centralized routing.** `tests/workstack/test-agent-routing.sh` covers project, harness, workflow, specialty, effort, independence, and fallback resolution from configuration.
8. [x] **SATISFIED — idempotent recovery.** `tests/workstack/test-resume.sh` covers completed-task, gate-head, existing-PR, and post-merge recovery without recreating durable state.
9. [x] **SATISFIED — closeout ownership and pruning.** `tests/workstack/test-active-contracts.sh`, `tests/workstack/test-plan-docs.sh`, and the Task 27 bounded repository audit recorded in `.superpowers/sdd/task-27-report.md` cover owners/prune triggers and confirm no Task 25/26 pilot artifacts or contracts exist to prune.
10. [x] **SATISFIED — divergence compatibility.** `tests/workstack/test-upstream-divergences.sh`, `python3 scripts/check-workstack-divergences.py`, `workstack/UPSTREAM-UPDATES.md`, and `tests/workstack/test-upstream-updates-doc.sh` protect and document the allowlist lifecycle.
11. [x] **SATISFIED — project guidance.** FSMCRM `AGENTS.md` contains two workflow paragraphs plus the three entry points; fork `workstack/AGENTS-SNIPPET.md` is enforced by `tests/workstack/test-workflow-summary.sh`.
12. [x] **SATISFIED — no hidden agent-only state.** The public procedure is described by `workstack/AGENTS-SNIPPET.md`, `skills/workstack-resume/{living-plan-format,plan-authoring,active-contracts}.md`, and `workstack/UPSTREAM-UPDATES.md`; their deterministic checks are `test-workflow-summary.sh`, `test-plan-docs.sh`, `test-active-contracts.sh`, and `test-upstream-updates-doc.sh`.
